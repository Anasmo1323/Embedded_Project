...
void Timer_Init(uint32 TimerBase, uint8 Channel, uint8 Mode, uint32 Prescaler, uint8 Edge)
{
    ...
    if (TimerBase == TIM2_BASE)
        Rcc_Enable(RCC_TIM2);
    else if (TimerBase == TIM3_BASE)
        Rcc_Enable(RCC_TIM3);
    else if (TimerBase == TIM4_BASE)
        Rcc_Enable(RCC_TIM4);
    else if (TimerBase == TIM5_BASE)
        Rcc_Enable(RCC_TIM5);
    else
...
uint8 Timer_CheckCapture(uint32 TimerBase, uint8 Channel, uint16 *CaptureValue)
{
    ...
    if (timer->SR & (1 << flag_bit))
    {
        ...
        {
        case 1:
            *CaptureValue = timer->CCR1;
            break;
        case 2:
            *CaptureValue = timer->CCR2;
            break;
        case 3:
            *CaptureValue = timer->CCR3;
            break;
        case 4:
            *CaptureValue = timer->CCR4;
            break;
        }
        // Clear capture flag
        timer->SR &= ~(1 << flag_bit);
...
uint32 Timer_GetCounter(uint32 timer_base) {
    TimerReg *timer = (TimerReg *)timer_base;
    return timer->CNT;
}
